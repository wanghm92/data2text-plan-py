...
PREPATH=$ENCODER\_$DATA\_new_edgedir-$EDGE_DIR\_newnorms
echo $DATA
echo $VER
echo $BASE
echo $PREPATH
...
TRAIN_EDGE=$BASE/train/edges_train.ncp.new.direction-$EDGE_DIR.newnorms.jsonl
...
VALID_EDGE=$BASE/valid/edges_valid.ncp.new.direction-$EDGE_DIR.newnorms.jsonl
...
TEST_EDGE=$BASE/test/edges_test.ncp.new.direction-$EDGE_DIR.newnorms.jsonl
...
####################################################################################################
# echo "run training"
# $PY3 train.py -data $PREPRO/roto-$PREPATH -save_model $OUTPUT/roto -encoder_type1 $ENCODER -decoder_type1 pointer -enc_layers1 1 -dec_layers1 1 -encoder_type2 brnn -decoder_type2 rnn -enc_layers2 $DEC -dec_layers2 $DEC -batch_size $BAT -feat_merge mlp -feat_vec_size $DIM -word_vec_size $DIM -rnn_size $DIM -seed 1234 -epochs 50 -optim adagrad -learning_rate 0.15 -adagrad_accumulator_init 0.1 -report_every 50 -copy_attn -truncated_decoder 100 -max_generator_batches 1000 -gpuid 0 -attn_hidden 64 -reuse_copy_attn -start_decay_at 4 -learning_rate_decay 0.97 -valid_batch_size $BAT -tensorboard -tensorboard_log_dir $OUTPUT/events -edge_aware $EDGE_AWARE -edge_aggr $EDGE_AGGR -encoder_graph_fuse $NODE_FUSE -encoder_outlayer $OUT_LAYER
