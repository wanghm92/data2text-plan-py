...
TRAIN_EDGE=$BASE/train/edges_train.ncp.new.direction-$EDGE_DIR.$EDGE_NORM.jsonl
...
VALID_EDGE=$BASE/valid/edges_valid.ncp.new.direction-$EDGE_DIR.$EDGE_NORM.jsonl
...
TEST_EDGE=$BASE/test/edges_test.ncp.new.direction-$EDGE_DIR.$EDGE_NORM.jsonl
...
#         echo $MODEL2
        ...
        # $PY3 evaluate.py --path $BASE --dataset valid --hypo $SUM_OUT/roto_stage2_$IDENTIFIER.e$EPOCH.valid.txt --plan $SUM_OUT/roto_stage1_inter_$IDENTIFIER.e$EPOCH.valid.min$MIN.txt
        # cd /mnt/cephfs2/nlp/hongmin.wang/table2text/$MYFOLDER
